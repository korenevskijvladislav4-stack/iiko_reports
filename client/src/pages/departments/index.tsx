    ...
    <div className="departments-page" style={{ width: '100%', maxWidth: '100%', paddingBottom: 24 }}>
      ...
      <Modal
        ...
        onOk={async () => {
          try {
            ...
            if (editingDept) {
              await updateDepartment({ id: editingDept.id, ...payload }).unwrap();
      ...
      >
        <Form form={deptForm} layout="vertical">
          <Form.Item name="name" label="Название подразделения" rules={[{ required: true, message: 'Введите название' }]}>
            <Input />
          </Form.Item>
          <Form.Item name="priority" label="Приоритет (меньше = выше)" tooltip="Меньшее значение отображается первым">
      ...
      <Modal
        ...
        onOk={async () => {
          try {
            ...
            if (editingPos) {
              await updatePosition({ id: editingPos.id, name: values.name, priority: values.priority }).unwrap();
            } else {
              await createPosition({ name: values.name, priority: values.priority }).unwrap();
            }
      ...
      >
        <Form form={posForm} layout="vertical">
          <Form.Item name="name" label="Название должности" rules={[{ required: true, message: 'Введите название' }]}>
            <Input />
          </Form.Item>
          <Form.Item name="priority" label="Приоритет (меньше = выше)" tooltip="Меньшее значение отображается первым">
            <InputNumber min={0} style={{ width: '100%' }} />
