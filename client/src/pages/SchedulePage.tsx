...
import { Card, Table, Button, Form, DatePicker, Select, Space, Typography, Modal, TimePicker, Input, Tag, Dropdown, message } from 'antd';
...
type Department = { id: string; name: string; priority?: number };
...
function enumerateDates(from: Dayjs, to: Dayjs): Dayjs[] {
  const dates: Dayjs[] = [];
...
function hoursBetween(startTime?: string | null, endTime?: string | null): number {
  if (!startTime || !endTime) return 0;
  const start = dayjs(startTime, 'HH:mm');
  const end = dayjs(endTime, 'HH:mm');
  if (!start.isValid() || !end.isValid()) return 0;
  let diff = end.diff(start, 'minute') / 60;
  if (diff < 0) diff += 24; // смена через полночь
...
export default function SchedulePage() {
  ...
  const departmentsSorted = useMemo(() => {
    return [...departments].sort((a, b) => (a.priority ?? 0) - (b.priority ?? 0));
  }, [departments]);
  ...
  const assignShift = async (userId: string, date: Dayjs) => {
    ...
    const existing = schedules.find(
      (s) => s.userId === userId && dayjs(s.date).isSame(date, 'day'),
    );
    const payload = {
      userId,
    ...
    try {
      if (existing) {
        await updateSchedule({ id: existing.id, payload }).unwrap();
      } else {
  ...
  const columns: ColumnsType<MatrixRow> = useMemo(() => {
    const base: ColumnsType<MatrixRow> = [
      {
        ...
        render: (_val: string | undefined, record: MatrixRow) => {
          ...
          return (
            <span style={{ paddingLeft: 28, color: 'var(--premium-text)' }}>
              {record.userName}
            </span>
          );
        },
    ...
    const dayCols: ColumnsType<MatrixRow> = days.map((d) => {
      ...
      return {
        ...
        onHeaderCell: () =>
          isWeekend
            ? {
                className: 'schedule-col-weekend',
                style: { background: 'rgba(100, 116, 139, 0.5)', color: 'var(--premium-muted)' },
              }
            : {},
        render: (value: string | number | undefined, record: MatrixRow) => {
          ...
          const schedule = schedules.find(
            (s) => s.userId === userId && dayjs(s.date).isSame(d, 'day'),
          );
    ...
    return [...base, ...dayCols];
  }, [filterForm.getFieldValue('period'), schedules, scheduleUsers, editMode, editStart, editEnd, canEditSchedule]);
          ...
          <Form.Item name="departmentId" label="Подразделение">
            <Select
              ...
              options={[
                ...
                ...(usersByDepartment.noDept.length > 0
                  ? [{ label: 'Без подразделения', value: '__none__' }]
                  : []),
              ]}
          ...
          <Button
            ...
            onClick={async () => {
              ...
              try {
                ...
                if (editingSch) {
                  await updateSchedule({ id: editingSch.id, payload }).unwrap();
                } else {
                  await createSchedule(payload).unwrap();
                }
                setSchModalOpen(false);
